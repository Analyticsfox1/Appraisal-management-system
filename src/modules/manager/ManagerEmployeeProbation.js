...
class ManagerEmployeeProbation extends Component {
	...
	EmployeeProbationList = () => {
		...
		getAllInProbationEmployees(obj ? obj.userId : null).then(response => {
			this.setState({
				probationData: response.data && response.data.InProbationEmployeesList ? response.data.InProbationEmployeesList : []
			})
		})
	}
			...
			<div className="dash_grid">
				...
				<main className="bg-white">
					...
					<section className="container-fluid">
						<h5 className="text-center mt-2 mx-auto user-box">Employee In Probation</h5>
						<div className="mt-4">
							<ReactTable
								data={probationData}
								columns={columns}
